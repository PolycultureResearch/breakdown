...
RCA_HOW_TO_READ = (
    ...
    "- Every number here is `analysis_window` measured against `reference_window`. Quote "
    "both, with dates, beside any figure you report, and say when `reference_defaulted` is "
    "true (the engine chose the reference, not the person asking): a result without both "
    "windows cannot be re-run, and the reference is the input the headline is most "
    "sensitive to.\n"
