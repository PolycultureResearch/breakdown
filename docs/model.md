...
The [statistics white paper](../knowledge/statistics_whitepaper.md) has the
*why* underneath: why each model was chosen, its strengths and weaknesses,
cited sources, and an assessment of the engine's overall statistical rigor.
...
level changes the parents don't capture.
...
recomputes from its components as `Σnumerator / Σdenominator`, never an average
...
- **Windows snap per node** to the whole periods fully inside the requested
  ...
  grain and the most recent whole period the data holds named in the error. Two
  ...
  `gap`, because those come from the data rather than the model; what is
  missing is the *split*, not the movement.
...
- **A `flow` metric that starts partway into the loaded window is filled with
  ...
  exist yet is not a metric that was zero, so the fill is logged with the
  periods it invented, and you should read that warning. The fit trains on the invented
  ...
  does. (`stock` refuses instead, since it has nothing to carry backwards.
  `rate` neither fills nor refuses: see below.)

### Rates over undefined periods

A rate whose denominator is genuinely zero in a period, because nobody churned
that week or nothing was on sale that day, has **no value** there. It is not
low, and it is not zero: `0/0` is undefined, and the canonical semantic-layer
idiom (`num / nullif(den, 0)`) returns NULL for exactly this reason. breakdown
carries that through rather than inventing a number, and the same
representation stands for a period the source simply never returned, because a
provider cannot tell the two apart. What it *can* do, once the rate declares
its `denominator`, is classify them after the fetch: denominator zero is a
fact, denominator non-zero is a missing value. The startup log says which.
...
- **Formula nodes** get exact **symmetric per-day** Shapley attribution. Both
  ...
  parents, while a covariance that is merely *present* but unchanged in both
  windows contributes nothing. For `revenue = orders × aov`, "the big orders
  disappeared" is exactly an orders–aov covariance shift, and it shows up in
  the attribution instead of in `unexplained`. (For non-product
...
and hold for any formula, evaluated per day. For the common case of a formula
`A × B` with exactly two parents, they collapse to a closed form worth having
memorized:
...
`mean(A×B over analysis) − mean(A×B over reference)`, with no remainder. That
is also why a real behavioral change like "the large orders disappeared"
...
enough replicates evaluated to a non-finite number that the interval was
computed from the survivors or withheld entirely; the cause is again a
near-zero denominator, this time hit only by some resamples. Treat it the same way: the
...
every replicate on one side, is the estimator running out of resolution, not
a measurement of certainty. Saturation is most likely exactly where the
...
- **`trend`** is the fitted level's forecast change: the analysis window lies
  ...
  simulation of new steps. **That is an understatement, not a design note**
  (open, roadmap S16): a random walk accumulates variance with every step past
  the fit end, so the honest interval should widen the further the analysis
...
- A **derived** node is a formula node with no `source` of its own, whose
  series *is* `formula(parents)`. Its `unexplained` is `0` **by construction**,
  and the response says so with `unexplained_status: "definitional"`. That zero
  is not a reconciliation: nothing was compared, because there is no
  independently measured series to compare against. Read it as *we did not
  check*, and if you want the check, give the node a `source`. (breakdown then
  verifies the identity against the fetched series at **load**, over the whole
  window, and warns with the worst periods, so drift is reported even where no
  RCA looks.)
...
assumptions) compares belief distributions, a valid and useful comparison. The
