...
- **`localized`** is the headline verdict: whether the leading slice's excess reaches `localization_threshold` of the gap with its evidence intact (not `noise_level`, shares not withheld). The UI's "*X carries N% of the gap*" line renders exactly this field. When it is `false`, narrate the gap as spread across slices rather than naming the top slice, which exists because ranking always produces a first row.
