...
about half an hour. You will build a four-metric tree from scratch, serve it,
look at it, and ask it why revenue moved. It is the same tree that ships with
the package as
[`breakdown/examples/jaffle_shop_tree.yml`](../breakdown/examples/jaffle_shop_tree.yml),
so at every step you can compare your file against a finished one.

You need no credentials: the tutorial tree uses the `mock` provider, which
...
Three fields here are the core of every tree you will ever write:
...
- **`priors`** states what you believe before seeing data, in business units:
  ...
  couple of points (`sigma: 0.02`). The prior is not decoration. It stabilizes
  the fit when history is short, and data updates it rather than discarding
  it. If you have no belief, omit `priors` and the edge gets a
...
- **`denominator: order_count`** says what it is an average *per*. With this
  declared, breakdown computes a window's AOV as Σrevenue / Σorders, the
...
While the server runs, `GET /meta` returns the machine-readable view of what
...
The first run takes a minute because it is fitting real Bayesian models for
the probabilistic nodes; repeat runs reuse them. How to read what comes back:
...
- **`unexplained`** is a first-class finding: the part of the move the modeled
  ...
  the tree", and that is the honest story.
- **`ranked_causes`** is a triage order, not a verdict. It says where to look
  first. On this synthetic tree, expect the two revenue parents to offset
  (shares past 100% are normal when parents pull in opposite directions) and
  neither leg's direction to be firmly established over a fortnight: fourteen
  observations is thin evidence, and the intervals say so.

Before trusting output on real questions, read
[docs/model.md](model.md), the practitioner's guide to what these numbers
mean and where the model's assumptions bend.
...
`breakdown doctor`, which walks the whole chain (manifest → profile →
connection → per-metric bindings) and tells you exactly which metrics
translate and which are refused, by name and reason.
...
run `doctor` before trusting anything. breakdown caches fetched series as
parquet snapshots next to the tree, so restarts are cheap and analyses re-run
reproducibly.

## Where to go next

**Slice a gap across a dimension.** Declare `dimensions` on a metric and the
RCA can tell you *where* signups fell: which region, which plan.
