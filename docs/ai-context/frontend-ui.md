...
**`escalated` is the one fit state that is good news, and it is rendered as such.** When a node's approximation fails the k̂ check, RCA and what-if discard it and re-fit with NUTS; the panel then leads with `↑ Re-fitted with NUTS` in the *ok* colour, not a ⚠. `khatNote()` (beside `ciStatusNote`) is the single lookup behind every surface — node card, RCA header, what-if card, what-if table, exported report — with the same unknown-status fallback: a band this build cannot name is printed verbatim rather than treated as fine. The export is the sharpest case: `caveatBlock` stamps every entry with ⚠, so the k̂ note is composed *outside* that list, because "we used the better sampler here" under a warning glyph reads as the opposite of what it says (the fifth rule).
