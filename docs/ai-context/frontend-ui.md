...
- Every contribution edge gets `rca-scope`: width scales `2 + 6·min(|share_of_gap|, 1)`, line opacity encodes P(same direction), and the label shows the share as a percent. The red/green hue is a *separate* pair of classes (`rca-up`/`rca-down`), added only when the child metric declares a `direction` — width and opacity are measurements and render on every contribution edge; the hue is a judgment and exists only where the tree made one. Until 2026-09-02 width and opacity were bound inside the hue selectors, so the largest share in an analysis (60.5%, on the demo tree) rendered as a default 2px structural edge whenever it landed on an undeclared child.
- Clicking a ranked-cause row applies `.pathhl` to the path from cause to target: an accent **underlay glow** (`underlay-*`, Cytoscape ≥3.21), never a recolor. The old amber repaint erased the good/bad hue on exactly the edges being inspected and borrowed the caveat channel's color (⚠/◌/levers) for a selection state.
- The legend gains the up/down swatches plus an edge-encoding row (width = share of gap, faint = uncertain) while the overlay is active.
...
2. **Ranked causes**: one row per cause — rank, metric name, horizontal score bar, "via <child>". Clicking a row selects the node and glows the path from cause to target (the `.pathhl` underlay — see *RCA overlay* above).
...
**The two windows are the headline, on both surfaces (issue #114).** Every number an RCA publishes is the analysis window against the reference window, and the reference is usually the engine's own choice, so a reader who loses it cannot reproduce the result — a field report recovered theirs by matching a recorded actual against re-runs. Both surfaces had carried the dates since 0.1.0 as one clause of a muted subtitle beside the provider and the timestamp, and the export's `<title>` named only the analysis window. `windowsHeadline*` in `disclosures.js` is the one wording: two labelled lines (**Analysis window**, **Reference window**) under the gap in the live card and under the `<h1>` in the export, the engine's authorship of the reference in words (`REFERENCE_DEFAULTED_NOTE`), the snapped whole periods beside the requested dates when the target's grain changed them, and the export's title carrying both windows so the file name says what it compares.
...
**The collinearity verdict prints its pass, unlike k̂ (roadmap S4).** `collinearityNote()` / `COLLIN_NOTE` mirror `khatNote()` — three states (`moderate`, `high`, `unavailable`) plus the same unknown-status fallback, one lookup behind the RCA header chip, the exported report's header line and its `caveatBlock` — but the metric tab additionally prints the `ok` case in the diagnostics row as `parents separable (max |r| = 0.51)`. The asymmetry is the point: a k̂ only exists when the run asked for an approximation, so its absence is the honest render, whereas *every* multi-parent node is checked for collinearity and silence there would be indistinguishable from a check that never ran. The header chip names the pair inline (`collinPairText`) rather than only in the tooltip, so a reader scanning a wide RCA does not have to hover every node to find which two parents the split is soft between. `fit_quality` is deliberately untouched by it — a collinear fit is a correct fit that is properly unsure, so tinting the node as broken would be the opposite of the finding.

**The posterior predictive verdict prints its pass too, and it is the one caveat that also moves the fit gate (roadmap S3).** `ppcNote()` / `PPC_NOTE` mirror `collinearityNote()` exactly — three flagged states (`moderate`, `severe`, `unavailable`) plus the unknown-status fallback, behind the RCA header chip, the what-if chip and block, the export's header line and `caveatBlock`, and a metric-tab diagnostics row that prints the `ok` case as `model reproduces its data`. `ppcStatText` names the failing statistic and its p-value inline, for the same reason `collinPairText` names the pair: a reader scanning a wide RCA should not have to hover to learn whether the failure is a floor violation or leftover autocorrelation. The one place it diverges from S4: a `severe` verdict sets `fit_quality: "suspect"`, and on a NUTS fit it is the *only* thing that can — so the metric card's suspect explanation is now a conditional, and its `severe` branch says the likelihood is wrong for the metric rather than repeating the R̂/divergence/ESS sentence, which would have named a cause that did not happen. That is a fifth-rule failure, not a cosmetic one: the payload would have been correct and the sentence beside it false.

**The posterior predictive check is drawn, not just stated (roadmap S10).** A *Posterior predictive check* section sits between the posterior and the Analyze controls, filled by `renderPpcBand(name, data)` from `GET /metrics/{name}/ppc` — the one payload in this UI that scales with the fitted window, which is why it is a second fetch rather than a field on `/metrics/{name}` (that route is issued once per fitted metric by `clearRCA`). Cached in `state.ppcCache` and dropped alongside `state.metricCache` in `runAnalyze` and after an RCA, because both replace the fit a band belongs to. The fetch is async and the tab is rebuilt wholesale on every node click, so it re-checks `state.selected` before painting; it skips the request entirely when `data.summary` is null, which is the same `_pick_fit` signal the server would answer with.

Four rendering decisions, each of which was a way to mislead. **Nothing is tinted by the verdict** — the band is the same grey on a `severe` node as on an `ok` one, because the geometry is the evidence and a red band on a node already labelled `severe` would be an illustration of the label rather than the thing behind it. **The zero line is drawn deliberately** (`zeroline` is a Plotly default, pinned here with a comment) because on a count or a rate the entire `min` finding is whether the band crosses it. **The y-axis uses `exponentformat: "power"` and every hovered number goes through `fmt()`**, because d3's SI notation renders a formula node's identity residual of 4.5e-13 as `400f` — read as four hundred of something rather than as machine epsilon, which is a correct payload made false by its own axis. **The key is HTML** (`.ppc-key`, the `.win-ref` swatch idiom) rather than a Plotly legend: five entries in a ~360px sidebar wrap to five rows that Plotly draws *inside* the plot area, over the densest part of the series.

The caption carries what the picture cannot: how many periods were fitted and through when (an RCA's fit stops before the analysis window, so the chart does too), whether the line is the metric or a formula node's residual, and the count of periods outside the 95% band — stated without a threshold or a status, because across four demo nodes that count reads 5.6 / 4.6 / 4.5 / 3.6% while their verdicts run `severe` / `moderate` / `ok` / `moderate`. It does not separate them; `ppc_status` is the verdict and this is a description of the picture. `ppcPanelNote` handles the three states that draw nothing — no fit, a withheld band with its reason, a failed fetch — in prose, because an empty chart frame reads as a clean one (rule 3).
...
| `GET /metrics/{name}/ppc` | the Metric tab's **Posterior predictive check** plot (roadmap S10) — a route of its own because it is window-scaled, cached in `state.ppcCache` and dropped wherever `state.metricCache` is |
