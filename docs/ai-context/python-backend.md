...
breakdown/
  ...
  loading.py       # tree + provider → aligned GrainedData: build_fetcher, fetch_all_metrics, the load-time checks
  ...
  api/
    main.py        # FastAPI app — routes, lifespan, the bearer-token gate, per-tree state wiring (composition moved to loading.py, roadmap grill M9)
...
`build_query(bind, grain=…, start_date=…, end_date=…, dialect=…, dimension=…, selection=…)`
...
### `fit_metric(dag, data, target, draws=NUTS_DRAWS, tune=NUTS_TUNE, inference_method="nuts", fit_end=None, chains=NUTS_CHAINS, random_seed=None, vi_iterations=ADVI_ITERATIONS) -> FitResult`
...
- `_prepare_series(defn, parents, data, target)` → `(y, X, scale, y_mean, y_std, x_stds, dates, fitted, dropped)`. Formula nodes fit the z-scored residual (X is None); probabilistic nodes get one z-scored regressor per parent, lag-shifted and trimmed by `max(lags)` (raises if < 10 rows remain); `scale[i] = x_std_i / y_std`. **A parent whose (shifted, trimmed) column has zero variance is dropped** (issue #113): a constant column is not identified and carries no information about the target's movement, so leaving it out cannot change the posterior on the rest. `fitted` is the parent axis of `X`/`beta`/`beta_raw` (DAG order minus the dropped), `dropped` the `{parent, reason}` records; both reach `FitResult.parents` / `FitResult.dropped_parents`, and a WARNING names each drop. Every consumer that indexes a coefficient — `fit_metric`'s own sign check, `rca.run_rca`'s posterior branch, `simulate.run_scenario`'s `beta_axis`, the API's `fitted_parents`, and `app.js`'s `labelBetaEdges` / `renderPosterior` — walks `fit.parents`, never `list(dag.predecessors(name))`, for exactly this reason. All parents constant still raises, with a reason in the author's terms.
...
- `_ppc_diagnostic(y, y_rep, mu, target, grain)` — roadmap S3, described under its own heading below. The draws come from `_posterior_predictive_draws(...)`, which must run **inside** the `pm.Model()` context; the verdict is attached afterwards as `ppc_status` / `ppc` / `ppc_warnings`. `_ppc_band(...)` runs on the same draws and keeps the per-period quantiles S10 plots, on `FitResult.ppc_band` rather than in `diagnostics` — see its own heading below.
- `_collinearity_diagnostic(X, parents, target, grain)` — roadmap S4, described under its own heading below. Computed on `X` *before* sampling (it is a property of the design matrix, not the trace) and attached afterwards as `collinearity_status` / `collinearity` / `collinearity_warnings`.
...
Inference: `nuts` → `pm.sample(draws, tune, target_accept=0.9, chains=chains)`; `advi`/`fullrank_advi` → `pm.fit(n=vi_iterations).sample(draws)`. Returns a `FitResult` (trace + normalization constants + fitted period-start `dates` + `grain` + `fit_end` + diagnostics).

#### The sampler budget is defined once, here (roadmap C27)

**`NUTS_DRAWS = 500` · `NUTS_TUNE = 1000` · `NUTS_CHAINS = 4` · `ADVI_ITERATIONS = 20_000`**, module-level constants above `fit_metric` with the reasoning beside them. Every other spelling reads them: `run_rca`/`run_scenario`'s `draws=` default, `POST /analyze/{name}`'s three `Query(default=...)`, and — checked rather than imported, because there is no build step — the numbers `app.js` prints and the defaults `docs/api-reference.md` documents.

It is one place because it was three, and they disagreed. `/analyze` declared `tune=500` while the analysis routes inherited `fit_metric`'s `tune=1000`; `fit_metric`'s own `draws=1000` was contradicted by the `500` every route passed. Warm-up is where NUTS adapts step size and mass matrix, so through v0.1.1 the same node over the same window returned a posterior from a different adaptation depending on which URL asked, with nothing in either payload saying which — a fit is meant to be a pure function of (DAG, data, target). It was harmless while `/analyze` was the only NUTS path and the analyses ran ADVI (which never reads `tune`); **S2's Option C put every route on NUTS and put both budgets on the same sampler.**

The values: `tune` standardized **up** to PyMC's own default, the more conservative of the two and the only direction that moves no RCA or what-if figure. Measurement did not pick it — 5 seeds × 4 probabilistic nodes of `knowledge/b2b_mrr_tree.yml` at 500 vs 1000 warm-up: two nodes better at 1000, two worse, and the seed-to-seed spread at *fixed* settings (3 to 196 divergences on one node) far larger than the gap between the budgets. So the tie broke on cost (~15–30% wall clock, on `/analyze` alone) and on the UI, which had been telling readers "after 1,000 discarded tuning steps" all along. `draws` standardized **down** to the 500 every route already ran; bulk ESS at 500 × 4 measured 213–921 against the 100 that flags a fit `suspect`, and doubling it would halve `MAX_CACHED_TRACE_BYTES`'s capacity (a NUTS fit is 27.0 MB — see the trace-cache note below). `vi_iterations` had *not* split — nothing overrides it and no route exposes it — and is hoisted anyway, because one spelling today is exactly what `tune` had before `/analyze` grew its own.

Enforced by `test_the_sampler_budget_is_defined_exactly_once`, `test_no_sampler_budget_is_written_as_a_literal` (AST-walks `engine/`, `api/`, `mcp/server.py`, `cli.py`, `doctor.py` for a literal `draws`/`tune`/`chains`/`vi_iterations` at any call site, parameter default or `Query(default=)`), `test_every_rendered_sampler_budget_matches_the_engine` (app.js) and `test_the_documented_route_defaults_are_the_route_defaults` (api-reference.md).

#### PSIS k̂, and why it is a disclosure rather than a trigger (roadmap S2)

Every variational fit is scored with **`_psis_khat(approx, n_draws, random_seed)`**, the diagnostic of Yao et al. (2018): treat `approx` as an importance-sampling proposal for the true posterior, form the log ratios `log p(θ, y) − log q(θ)`, and fit a generalized Pareto to their tail (`arviz.psislw`). The shape parameter k̂ is how heavy that tail is, i.e. how far q sits from p.

Both terms come from PyMC's own symbolic machinery, in the model's **unconstrained** space where q lives: `approx.set_size_and_deterministic([approx.sized_symbolic_logp, approx.symbolic_logq], size, 0)` clones both against the *same* draws, so the ratio is a ratio of comparable densities on one sample. `sized_symbolic_logp` is `model.varlogp + model.datalogp` with the change-of-variables Jacobian; `symbolic_logq` is whatever the fitted family supplies — which is the reason for going through it rather than reconstructing the variational density by hand: mean-field and full-rank each answer for themselves and the diagnostic needs no branch. Seeding mirrors `Approximation.sample_dict_fn`: `reseed_rngs(find_rng_nodes(...), random_seed)`.

Bands, and why three: `_KHAT_GOOD = 0.5` (finite variance — close), `_KHAT_UNUSABLE = 0.7` (the conventional PSIS bar: past it neither the variance nor, past 1, the mean is finite, and reweighting cannot rescue it). `khat_status` is `ok` / `suspect` / `unusable` / `unavailable`. **`unavailable` is rule 3**: a non-finite k̂, a Pareto fit that failed, or too few finite ratios yields `khat: None` plus a named status and a reason — never a NaN on its way to `allow_nan=False` JSON or to `round_floats`, which would render it `null` and read as "no verdict".

`fit_quality` stays the two-valued gate and goes `suspect` when *either* check fails, so every existing consumer's `=== "suspect"` test keeps working; k̂ is the evidence beside it, the way `max_rhat` is for NUTS. An `unavailable` k̂ does **not** flip it — unchecked is not failed — but it does carry a `khat_warnings` entry saying so, because silence reads as a clean bill of health.

#### k̂'s own error, and the seed that stopped it wandering (roadmap S22)

k̂ is fitted to the tail of `_KHAT_DRAWS = 1000` sampled importance ratios, so it is an estimate with sampling error, published — until 2026-08-27 — as a bare number. Two independent fixes, and the roadmap row names them (a) and (b).

**(a) One seed, read from one place.** `run_rca` and `run_scenario` each wrote `random_seed=0` at their own `fit_metric` call site and `POST /analyze/{name}` passed nothing, so the manual-fit route returned a different posterior — and a different k̂ — from two identical requests (measured on the White Cube tree at full window, 2026-08-27: `customer_churn_rate` 1.23 then 1.91, `trials_started` 0.94 then 1.19; the same pair had measured 1.23/0.79 and 0.89/0.77 five days earlier, which is the point). All three now pass **`FIT_RANDOM_SEED`**, defined beside the four sampler budgets and for the same reason, and `test_every_fit_the_engine_runs_for_a_caller_is_seeded` AST-walks the package for a `fit_metric(...)` — direct or through `asyncio.to_thread` — that does not pass it. `fit_metric`'s own default stays `None`: a library caller may want an unseeded fit; a *server* may not answer the same question two ways.

**(b) The error, and what it means for the bands.** `_khat_se(khat, log_ratios)` is the generalized Pareto shape parameter's asymptotic standard error, `(1 + k̂) / √M` over the M tail points ArviZ actually fits (Smith 1985; Hosking & Wallis 1987), scaled by `M / (M + _GPD_PRIOR_K)` because `arviz.stats.stats._gpdfit` is an empirical-Bayes estimator that shrinks toward 0.5 with prior weight 10 and shrinks its variance with it. M is recomputed with `_khat_tail_len`, mirroring ArviZ's own cut (`ceil(min(n/5, 3√n))`, floored at `log(tiny)`) rather than assumed from n — 1000 draws give **95** tail points, not the 200 an earlier comment here claimed. Checked against what it describes rather than trusted: holding one approximation fixed and re-estimating k̂ over 60 independent draws, published-se ÷ empirical-sd came out within ~10% of 1 on four posteriors with k̂ from 0.03 to 1.15 (the exact ratios are canonical in the [white paper](../../knowledge/statistics_whitepaper.md)'s S22 revision entry — the 2026-08-29 grill found this figure restated at two precisions across four files, and a measurement quoted in more than one live document rots in one of them) (`test_the_published_khat_error_is_the_error_the_estimate_actually_has` runs the 20-replicate version).

The error is **large**: ~0.15 near the 0.5 bar, ~0.2 near k̂ = 1 — most of the width of the `suspect` band itself. Buying it down costs n^(−1/4) (the tail grows as √n), so halving it means 16× the draws; S22 reports the error instead, and `_KHAT_DRAWS` is the constant that would move if that is ever revisited. `khat_se` is `None`, never `0.0`, where the asymptotics do not apply (k̂ ≤ −0.5, or a tail ArviZ itself gives up on) — rule 3 one field over from where S2 applied it, and a zero would be worse than a null because it would make every band edge look resolved.

**`khat_borderline`** is the consequence: `min(|k̂ − 0.5|, |k̂ − 0.7|) < khat_se`. `khat_status` deliberately keeps meaning *which band the point estimate is in*, so no consumer has to reinterpret it and the UI's "PSIS k̂ is above 0.5" tooltips stay true; the flag is the separate statement that the band is not resolved. It **does** flip `fit_quality`, including from an `ok` k̂ — the gate asks whether the fit can be trusted as-is, and "not shown to be far from the posterior" is not an answer to it. The demo tree has a live case: `trial_conversion_rate` scores 0.854 ± 0.172, `unusable` on the point estimate and unable to separate `unusable` from `suspect`.

The uncertainty rides *inside* the single `khat_warnings` sentence rather than as a second entry, because every surface renders that list verbatim and a surface showing entry two without entry one would quote the caveat without the number. In the UI, `khatNote()` takes the **node** rather than the bare status for the same reason the flag exists: a borderline `ok` would otherwise render nothing at all, which is the fifth rule's failure mode exactly.

**No escalation, and that is the finding rather than an omission.** The first cut of S2 re-fitted an `unusable` node with NUTS behind the caller's back, capped at four per analysis. Then the measurement came in: mean-field fails the check on *essentially every real node* (k̂ far above every threshold on the demo trees — the exact per-node figures are canonical in the [white paper](../../knowledge/statistics_whitepaper.md)'s S2 entries), which made "escalation" the common path rather than a rescue — and the cost argument that justified ADVI collapsed alongside it (the 106-metric B2B tree holds exactly five probabilistic fits, and fitting all five measured 31.3s ADVI against 28.0s NUTS — the exact sampler was the faster of the two). So the default moved to NUTS everywhere and the escalation machinery came out: `MAX_ESCALATIONS`, `escalation_budget_warning`, `escalate_on_khat`, the `escalated` status and the "re-fitted with NUTS" UI badge are all gone.

What k̂ does now is make the opt-in honest: `inference_method` is a promise about which sampler runs, and on the `advi` path the caller has explicitly bought speed, so spending 2-20× to escalate is exactly the trade they declined. The remedy travels in `khat_warnings` instead — re-run without `inference_method=advi`, or upgrade one node with `POST /analyze/{name}?inference_method=nuts`.

**`cached_fit_is_usable(fit, inference_method)`** is the one place the reuse rule lives, imported by both orchestrators so the two cannot drift. Reuse is allowed only *upward*: a cached NUTS fit answers an `advi` request (it is the thing being approximated, and reusing it is free), a cached approximation does **not** answer a `nuts` request. `traces` is shared by every viewer of a process, so without this one colleague's deliberate `?inference_method=advi` triage run would silently decide the sampler behind everybody else's default analysis of the same window. It is the read-side twin of `_fit_rank` in `api/main.py`, which orders the same two methods on the write side.

#### Posterior predictive checks: is the model right for the data? (roadmap S3)

**`_ppc_diagnostic(y, y_rep, mu, target, grain)`** answers the question R̂, ESS, divergences and k̂ all leave open: *could this fitted model have generated the series it was fitted on?* Convergence says the sampler explored the posterior; k̂ says a variational approximation is close to it; neither is about the model. A misspecified node passes both in silence as long as it converges (white paper §3.2 #5).

Four test statistics — `min`, `max`, `resid_max`, `resid_acf1` — each scored as a two-sided posterior predictive p-value with a **mid-p** correction. Two bands: `moderate` at p < 0.10, `severe` at p < 0.02, deliberately wider than the conventional 0.05/0.01 because four statistics per node cross a 0.05 bar 19% of the time by chance and these are disclosure triggers rather than hypothesis tests.

Three implementation decisions are load-bearing and easy to undo by accident:

1. **`sd` is not a statistic, and must not become one.** `_prepare_series` z-scores `y` and `sigma_obs` is free, so the replicated standard deviation matches the observed by construction — measured p = 0.479 / 0.499 / 0.521 across a well-specified, a t(3) and a Poisson world. A statistic that cannot fail reads as a check that passed. `test_ppc_omits_the_dispersion_statistic_that_cannot_fail` pins this.
2. **`mu` is reconstructed in numpy, not stored as a `pm.Deterministic`.** It is an exact function of `alpha` / `trend` / `beta` plus the seasonal coefficients, all already in the trace. Adding it to the graph would put a second trend-sized array on every trace forever, in a cache whose cap is measured in gigabytes (rule 2).
3. **The posterior is thinned to `_PPC_DRAWS` before replicates are drawn**, not after — a 4-chain, 1000-draw trace over an 830-period window is 3.3M floats of replicates and as many again of `mu`, on every fit, for precision the p-value cannot use.

Unlike S4 it **does** move `fit_quality`, but only on `severe`, and the policy is written as `_ppc_moves_fit_quality(status)` rather than an inline comparison so it is greppable and directly testable. The asymmetry is the finding: a collinear design produces a *correct* fit that is properly unsure about the split, while a model that cannot generate its own data is not correct. `moderate` stays out of the gate — wiring it there would make `suspect` the common verdict and drain it of meaning — and `unavailable` stays out because an unchecked model is not a failed one (the rule S22 applied to k̂'s borderline band). Because `severe` is the only thing that can set `suspect` on a NUTS fit, `app.js`'s suspect-explanation branch had to become conditional: it previously said "one of R̂, the divergence count or the effective sample size crossed the threshold" unconditionally, which would now name a cause that did not happen.

Threading follows S4 exactly — `model.py` → `rca._node_out` (`ppc_status` / `ppc` / `ppc_warnings`) → `simulate.py` (verdict + sentences, numbers left on the fit) → `mcp/shaping.compact_rca` and `compact_scenario` (verdict always, per-statistic evidence only when not `ok`) + `RCA_HOW_TO_READ` / `WHATIF_HOW_TO_READ` + `mcp/server.explain_metric` (evidence in full) → `app.js` (`ppcNote` / `PPC_NOTE`, RCA header chip, what-if chip and block, the export's header line and `caveatBlock`, and the metric card's `bits` row where the `ok` case prints so "reproduces its data" and "never checked" cannot look alike).

#### The band behind the verdict, and where it is allowed to live (roadmap S10)

**`_ppc_band(y, y_rep, dates, y_mean, y_std, is_residual, target)`** runs beside `_ppc_diagnostic` on the same draws, and keeps what that function summarizes away: five quantiles of `y_rep` per fitted period (`lo95`/`lo50`/`median`/`hi50`/`hi95`), the observed series, the `dates`, and the indices where the observed value falls outside the 95% band. All in the **raw units of the fitted quantity**, all JSON-safe, all withheld together under a named `reason` if anything is non-finite — a hole in a band reads as certainty, which is the opposite of not knowing (rule 3).

Two placement decisions carry the item:

1. **It is a field on `FitResult`, not a key in `diagnostics`.** `diagnostics` is returned whole by `GET /metrics/{name}` and its `ppc` block is copied onto every RCA node and shaped into every MCP payload. This array scales with the fitted window — 88 kB of JSON on a 790-day node against ~600 bytes for the verdict — so riding there would put chart data on each of a 106-metric analysis's nodes and hand it to an agent that cannot see a chart. `test_no_per_node_payload_carries_a_series` enumerates rather than pinning `ppc_band` by name, since the next author to want a per-period array will not call it that.
2. **`_trace_nbytes` counts it, and now counts `dates` too.** The store measures the trace's xarray groups and, before this, nothing else — so two per-period terms sat outside the byte budget that is supposed to bound the thing that grows (rule 2). Each was individually small enough to justify skipping, which is the `slice_cache` argument verbatim. `summary_json` remains a *named* exception, because it is filled lazily on the first `GET /metrics/{name}`, after the store already weighed the entry; `test_every_window_scaled_field_on_a_fit_is_metered_or_named` enumerates `FitResult`'s fields against the three categories so a new one cannot join silently.

Recomputing on demand was rejected before either: `sample_posterior_predictive` needs the model *graph*, not the trace, so a route would have to refit the node — a minute of NUTS to redraw a chart, and a second posterior that is not the one the published p-values came from.

Served by **`GET /metrics/{name}/ppc`**, which picks the fit with the same `_pick_fit` the metric route summarizes (two routes choosing independently is how a chart comes to illustrate a verdict about a different window) and answers 200 in all three of its states, because "this node has no band" is information rather than an error.

#### Parent collinearity: naming the ridge k̂ can only detect (roadmap S4)

**`_collinearity_diagnostic(X, parents, target, grain)`** answers the one question a wide posterior cannot answer for itself: *which* parents the model cannot tell apart. Correlated regressors leave the likelihood almost flat along a ridge, so the data pins the parents' **sum** and says little about the **split** — and per-parent `contributions` / `share_of_gap` are exactly that split. Since S2 made NUTS the default, the interval on each parent is honestly wide; nothing said the two widths were one ridge measured twice.

It runs on `X` as the model sees it — z-scored, lag-shifted, trimmed, in `list(dag.predecessors(target))` order — so a lagged parent is correlated at its lag. Two channels, because they catch different shapes: pairwise |r| cannot see `x4 ~ x1 + x2 + x3` (every pair near 0.58, the design degenerate), and VIF cannot say which parents to look at. VIF is computed only for **three or more** parents; with two it is identically `1/(1 − r²)` and would restate the pairwise number.

Two bands each, for the reason k̂ has three — "the split is softer than the sum" and "the split is not a measured quantity" ask different things of a reader: `_COLLINEAR_R_MODERATE = 0.7` / `_COLLINEAR_R_HIGH = 0.9` (Dormann et al., 2013 for the first; ≥ 81% shared variance for the second), `_COLLINEAR_VIF_MODERATE = 5.0` / `_COLLINEAR_VIF_HIGH = 10.0` (Kutner et al., 2004). The VIF bars are deliberately looser than the pairwise ones convert to (0.7 → VIF 1.96, 0.9 → 5.26) because VIF is a *joint* quantity over k−1 regressors, so a wide node raises it for honest reasons. The demo tree is what set the moderate bar: `trial_activation_rate` / `trial_days_active` measure |r| = 0.863 over the 62 weeks an RCA fits them on, and a single 0.9 trip-wire would have passed in silence the node whose split moves 1.6 points across BLAS implementations from one seed.

**`unavailable` is rule 3**: a non-finite or constant column gives a 0/0 correlation, so the whole check is withheld under a named status with its reason rather than emitting NaN (a 500 through `allow_nan=False`, a `null` through `round_floats`) or a zero, which would read as "these parents are unrelated" — the most dangerous of the three. A parent that is an exact linear combination of its siblings gets `{"vif": null, "status": "unbounded"}` for the same reason, with the finding kept.

It never moves `fit_quality`, and the asymmetry with k̂ is deliberate: a bad approximation is a bad fit, while a collinear design produces a *correct* fit that is properly unsure. What is unsafe is reading one parent's number alone, which is what `collinearity_warnings` says. The threading follows `sign_warnings` exactly — `model.py` → `rca._node_out` (`collinearity_status` / `collinearity` / `collinearity_warnings`) → `mcp/shaping.compact_rca` (verdict always, evidence only when not `ok`) + `RCA_HOW_TO_READ` + `mcp/server.explain_metric` → `app.js` (`collinearityNote` / `COLLIN_NOTE`, the RCA header chip, the export's caveat block, and the metric card's `bits` row where the `ok` case is printed so "separable" and "never checked" cannot look alike).
...
**Reference-window sensitivity (roadmap S23).** `run_rca(..., reference_sensitivity=True)` ends by calling `_reference_sensitivity`, which builds `_reference_alternatives` — the published block shifted back by one period (7 days, or a calendar month when the scope's coarsest grain is month) and by one whole block length, each clamped to `_earliest_readable_reference` (the same floor the default respects) and dropped with a reason when nothing readable remains — and re-runs `run_rca` under each with `reference_sensitivity=False`, `progress=None` and the *same* `traces` dict. No alternative ever fits: the fit window is independent of the reference, so every node's cached `(name, analysis_start)` trace is reused and the added cost is the bootstrap and the Shapley games again, twice (`REFERENCE_SENSITIVITY_SHIFTS`; rule 4's cap sits one level down on the games themselves). The result is the top-level `reference_sensitivity` dict: `status` ∈ `REFERENCE_SENSITIVITY_STATUSES` (`stable` / `unstable` / `unavailable`), `top_cause` (the published `ranked_causes[0]`), `top_cause_stable`, `gap_sign_stable` (gap direction judged at the node's own scale via `negligible_gap`, C5's epsilon), `gap_range` over the published and every answered alternative, `reason`, and `alternatives[]` each with `shift`, `label`, `reference_window`, `status` (`ok` / `unavailable` / `gap_unavailable`), `reason`, `gap`, `top_cause`, `note`. Rule 3: an engine refusal of an alternative block (`ValueError` — coverage, an undefined target, no whole period) is caught and becomes that alternative's `reason`; a non-finite gap is withheld as `gap_unavailable`; an undefined published gap makes the whole field `unavailable`. No published number changes. `compact_rca` keeps the field with per-alternative nulls trimmed; `RCA_HOW_TO_READ` carries the reading rule.
...
### `run_rca(dag, data, traces, target, *, analysis_start, analysis_end, reference_start=None, reference_end=None, inference_method="nuts", draws=NUTS_DRAWS)`

Root cause analysis over `nx.ancestors(dag, target) | {target}`. `traces` is the caller's cache (`app.state.traces` in the API); missing probabilistic fits are added to it in place (`fit_end=analysis_start`, keyed `(node, analysis_start)`). `inference_method` defaults to `nuts` and is passed to `fit_metric` unchanged — this path never substitutes a sampler in either direction. `advi_draws` was renamed to `draws` when the default stopped being variational; it means posterior draws per chain under NUTS and samples from the fitted approximation under ADVI, exactly as on `fit_metric`.
...
1. **Fit what's missing.** Probabilistic (non-formula, non-root) nodes in scope without a *usable* cached trace (`cached_fit_is_usable`, above) are fitted — skipped when their windows hold no whole period at their grain. The fit is cached under `(node, analysis_start)`, so a repeat analysis of the same window pays nothing. The write is unconditional and safe: a node reaches the loop only when nothing usable was cached, so the fit being stored is never worse than the one it replaces.
...
| `fit_failed` | the node's own `fit_metric` raised — the node's own series with **no variance across the fit window**, *every* parent held flat (the reason names them), or an undefined period inside it. A single flat parent no longer raises (issue #113): it is dropped and named on `dropped_parents` | `grain`, `effective_windows`, `baseline`, `actual`, `gap`, `relative_change` — these are read off the data, not the model, so only the attribution is missing |
...
Ranking is by **excess concentration**: `excess_g = contribution_g − baseline_share_g × gap` (Σ excess = 0), with `ci_95`/`prob_concentrated`/`noise_level` from the same seeded circular moving-block bootstrap as RCA (`_block_bootstrap_indices`, `BOOT_BLOCK`, joint resampling across slices; single-period windows withhold CIs via `ci_status`). Cardinality: top-`top_k` by mean |value| (or the `values` pin-list), rest folded into a full `__other__` row (sum of *fetched* non-top slices — never `unsliced − Σ topK`); hard cap 100 distinct values. **Where the fold happens is the provider's call (roadmap C32).** `_select_slices` treats a column already named `__other__` as a provider's own roll-up — never a candidate, always folded into the same `__other__` — so a frame the warehouse folded and a frame fetched whole meet in one column. The frame says which it was in `df.attrs["slice_rollup"]` (`data_fetch.SLICE_ROLLUP`): `{where: sql, n_distinct, n_folded}` feeds the cardinality gate and `n_values`, anything else means the engine folded. The API's `_run_slice` builds the `SliceSelection` (`top_k`, `values`, the two grain-snapped windows — exactly `all_dates`), asks the fetcher `slice_rollup_refusal(metric, dimension, kind, weight)` once so a rate and its weight fold the same slices, keys the slice cache on the selection, and publishes `rollup` with the refusal sentence as `reason` when the fold was client-side. `dbt_sql._rolled_up` is the SQL half: rank by Σ|value| inside the rank windows (a ratio by Σden), keep every slice within 1e-9 of the k-th volume so the client breaks ties with its own arithmetic and name order, fold the rest as SUM (Σnum / Σden, 0 where Σden is 0, for a ratio), and return the kept rows with the dimension's raw value plus `bd_other`/`bd_n_distinct`/`bd_n_folded`, which `DbtDataFetcher.fetch_metric_sliced` turns into `__other__` and the attribute. Refused, by name: stocks (forward-filled before ranking), rates not bound as `agg: ratio`, and rates whose `weight` is not structurally the ratio's denominator (`_weight_is_denominator`). `SnapshotFetcher` serves a whole-frame hit as stored (attribute `where: client`, reason names the snapshot) and never writes a rolled frame; `BREAKDOWN_SLICE_ROLLUP=client` turns the pushdown off. `tests/test_slice_rollup.py` runs both paths on the same DuckDB data and asserts frame and result equality, including ties, pins, a NULL slice and the ratio fold. A `reconciliation` block compares Σ slices (or the per-date weighted blend) against the metric's own series — `"discrepant"` above 0.5% of |baseline| is a caveat, never a silent correction. Windows snap per the metric's grain exactly as in RCA; slicing a lagged parent means the caller passes the parent's lag-shifted windows.

The **headline verdict** is decided once, in `_localization`, and published as `localization` (`"localized"` | `"long_tail"` | `"not_localized"`) plus `localization_threshold` and the older boolean `localized`; the UI, MCP's `compact_slice` and the demo suite's `ui_localized` all read those fields rather than re-deriving the rule, which is what C24 was. Two gates, not one: the leader must clear `localization_threshold` (0.25) of the gap with its evidence intact (not `noise_level`, `share_of_gap`/`baseline_share` not withheld), **and** it must be a value someone declared. `__other__` clearing the bar is `long_tail` instead — the tail really moved, so suppressing it to `not_localized` would understate, but it is the roll-up of the values nobody enumerated, so naming it as the culprit points the reader at a segment that does not exist (roadmap 2.21). A verdict whose next move is a *setting* rather than a segment has to name the setting, so `long_tail` also publishes `localization_remedy` (raise `top_k`, pin `values:`, or slice another dimension) — one sentence, in the same dict, rendered by the panel and read by an agent. Deliberately not a caveat: the panel already prints the verdict, and the same paragraph twice on one screen reads as two findings. `localized` stays the narrow "may I print *X carries N% of the gap*?" boolean and is therefore `false` under `long_tail`: a consumer written before 2.21 is restrained rather than misled.

---

## `engine/simulate.py`

### `run_scenario(dag, data, traces, scenario, inference_method="nuts", draws=NUTS_DRAWS, n_draws=2000)`
...
**Fitted mode** (`data` present): baselines are `window_mean` over `baseline_start/end` (required; whole periods at each node's grain, no whole period → `ValueError`); coefficient draws index each needed node's `beta_raw` posterior, fit on demand with `inference_method` (default `nuts`, same reuse rule as `run_rca`) into the caller's `traces` cache (keyed `(node, fit_end)`, `fit_end = baseline_end + 1 day`, or the full-window `None` key when the baseline runs to the data edge). Extrapolation honesty flags compare simulated levels against full-history min/max and ±2σ. **The `non_physical` flag is separate and structural** (roadmap C26): `_non_physical_warning` is the one place a simulated value is called impossible, and it reads `_STRUCTURAL_BOUNDS` — today `share: true` → `[0, 1]` — before falling back to the conservative historical floor (`simulated < 0` on a metric whose `hist_min >= 0`). The declared bound applies on *both* sides and in *both* modes and never touches `hist`; the historical floor is an inference about the sample and deliberately has no ceiling counterpart, because "never observed above X" is `extrapolation`'s claim, not this one. `share` cannot be read off `denominator` — `average_order_value` declares one and is ~$182 an order — nor off `format.style: percent` (net dollar retention exceeds 100% by design) nor off `plausible` (which means plausible, not possible); the enumeration and its two-sidedness are pinned in `tests/test_project_invariants.py`.
...
Response contract: `{"mode": "fitted"|"cold_start", "baseline_window" (null in cold-start mode), "n_draws", "seed", "sources", "nodes", "warnings", "caveats"}`. Per-node: `status` (`baseline`|`affected`|`intervened`), `baseline`, `simulated`, `delta: {estimate, ci_95}`, `relative_delta`, `prob_direction`, `fit_quality`, `khat_status`, `khat_warnings` (both null on the NUTS default), `collinearity_status`, `collinearity_warnings` (roadmap S4, null below two parents — a scenario is a *lever*, and one the fit cannot separate from its neighbour is not one you can pull alone), `ppc_status`, `ppc_warnings` (roadmap S3 — a lever whose model cannot reproduce its own history is not one whose magnitude means anything), `extrapolation`, `non_physical` (bool — the sentence for it lives once, in `warnings`, keyed by metric), `contributions`; cold-start mode adds `baseline_ci_95` (null for point baselines) and swaps `CAVEATS` for `COLD_START_CAVEATS`. Seeded rng (`seed: 0`) with fixed draw order — identical calls are byte-identical.

Coefficients are read against **`beta_axis[node]`** — the fit's own `parents` list in fitted mode, the DAG's in cold start — never `list(dag.predecessors(node))` directly, because a fit that dropped a constant parent (issue #113) has one column fewer than the DAG has parents. A scenario whose affected cone includes a dropped parent of a fitted node is refused by name (`ValueError` → 422), the same policy as the fit-raised refusal beside it: propagating zero across an edge with no coefficient would be a confident wrong number on every node downstream. A dropped parent outside the cone is harmless and ignored.
...
Dates are validated (ISO format, start ≤ end) both at the CLI and in `lifespan`. `load_tree(tree)` builds the fetcher from the tree's `provider` config and fetches every metric for the window **at its declared grain/kind**, assembling per-grain frames via `build_grained` (an **outer** join on `date` within each grain only — a monthly metric no longer drops daily rows tree-wide, and since per-metric windows a short series no longer narrows its siblings either). For `local`/`cloud` the queried metric name is the last segment of `source`, renamed to the tree `name`; mock generates by tree name directly.

`build_grained` requires each metric's own frame to be a **gap-free run of periods** (`_check_contiguous`, grain-aware via `_FREQ`, run per metric *before* the join), raising with up to 10 named missing dates plus a count; the grain frame is then reindexed onto the full run from the earliest start to the latest end, so it is gap-free by construction and a metric is `NaN` outside its own range. This is not tidiness: everything downstream indexes by position — the model's `t = arange(len(y))` dates the rows, lags shift by *rows*, and the bootstrap resamples contiguous runs — so a hole compresses the calendar and silently shifts every date rather than failing.

**Per-metric windows (GitHub #112, 2026-09-15): each metric keeps its own range, and a short series bounds only the analyses that read it.** The within-grain join was inner until then, so a series shorter than its siblings at either end bounded the shared frame for every metric at the grain — a frozen ad feed clipped a production tree's day grain to one period, the tree loaded, `/health` was ok, and every RCA failed later with "reference window not fully covered" (#121 named the metric; this made the clip unnecessary). Now `build_grained` outer-joins, records each metric's own first and last period on `GrainedData.span_of`, and `series(m)` slices the grain frame to that range, so no consumer of a series ever sees the `NaN` outside it. `fit_frame` keeps its inner join, and that is now the *only* inner join in the pipeline: a fit's window is the intersection of the node's range and its parents' — `MIN_FIT_PERIODS` is counted on it, `FitResult.dates` records it, the RCA node payload's `fit_window` and `GET /metrics/{name}`'s new `fit_window` report it. `derive_series` already inner-joined its parents, so a derived formula node is undefined outside the range every input covers (roadmap 1.11's representation, not a zero). RCA's `_validate_coverage` names the series that stop short of a window (`GrainedData.spans_at`), so the refusal reads "`paid_spend` runs [06-01, 08-08]" rather than citing the grain; a target that does not read the short metric is unaffected, and its payload is byte-identical to the same tree with the feed intact. What is left to disclose is narrower and still worth it: `_report_short_series` logs **one WARNING per grain** naming every metric that stops before the grain's reach, its edge and the periods short, and records the same onto `GrainedData.short_series` (`{grain: {trailing | leading: {reach, short: {metric: {ends | starts, periods}}}}}`, JSON-safe, `{}` when clean), which `/meta` and `/health` carry always and MCP `get_tree` only when non-empty. Nothing is filled: rule 1, the boundary naming what is absent rather than approximating it. `_data_through` (the tree-wide as-of date on `/health`, the node cards and the goal progress) stays the *min* of the metrics' edges on purpose — a stale feed is what a monitor is there to catch — and `/health` now names the metric holding it there (`data_through_bounded_by`); `/meta` adds `data_from` per metric as the other end of its range. The leading-gap *fill* in `_align_to_spine` (C18) used to rest partly on the inner join — trimming one node's leading run would have deleted those periods for every sibling — and now rests on its first argument alone; whether to trim instead is an open policy question, noted in that docstring.

**A `sparse: true` flow is filled at every edge by declaration, and the fill is recorded** (`_align_to_spine(..., sparse=True)`, GitHub #112's third suggestion). The declaration is a `MetricDefinition` field, refused by the parser on a stock, a rate or a derived node, and reaches the alignment contract as an opt-in keyword on `fetch_metric` — sent only when set (`sparse_kw`), so every stub and third-party fetcher keeps the signature it had. Under it the trailing run is filled rather than trimmed, and the leading/interior fills are counted rather than warned about as invented: the returned frame carries `attrs["sparse_fill"]` (`first_row`, `last_row`, per-edge counts, `whole_window` for a no-rows source, `filled`), `_align_to_spine` logs one INFO line per metric, and `fetch_all_metrics` lifts the records where `filled > 0` onto `GrainedData.sparse_fills`, which `/meta`, `/health` and MCP `get_tree` hand over verbatim beside `short_series`. `SnapshotFetcher` applies the flag on read too (`_realign_snapshot`), so a pre-declaration snapshot fills its tail on the next load; a snapshot written with the fill stores those zeros as rows and needs a refresh if the flag is removed. The fill is attributable by design — a tail of declared zeros is what a stale feed on the same metric would look like, and `last_row` is what lets a reader tell them apart.

**Two checks run over the loaded data, once, before anything is served.** `_check_identities` compares every *fetched* formula node against its own identity over the whole loaded window (derived nodes are skipped — there is nothing to check them against), warning above `_IDENTITY_DRIFT = 0.01` of the node's level; `unexplained` reports the same thing, but only for the windows somebody happens to analyse. `_check_declared_shares` (roadmap C26) is the same idea for `share: true`: the declaration is what lets a what-if call a value *impossible*, so a wrong one is a confident refusal of a possible scenario — and if the node's own loaded history already leaves `[0, 1]`, the log says so and names the range. Neither refuses. Both are the author's disagreement to resolve, and the parser cannot know which side is wrong.
...
**The forkserver preload.** `warm_inference_imports()` also calls `preload_forkserver_modules()`, and so does `fit_metric` just before `pm.sample`. Python 3.14 made `forkserver` the Linux default start method, and PyMC runs each chain in its own process, so without a preload every fit re-imports `pymc` once per chain — CI measured 3.14 at 2-3x the 3.13 fit time on identical packages, and ~1.45x with `pymc` preloaded (PR #111, 2026-09-02; the remainder is pickling the step method to the workers, which `fork` never had to do). `fork` was not restored because Python left it for real deadlock reasons in multi-threaded parents, and the API fits under `to_thread`. The preload list is read once, when the forkserver starts, which is why both call sites exist: the server's warm-up comes first in production, and the pre-sample call covers the CLI, tests and anything else that fits without a lifespan. `test_warm_inference_imports_preloads_pymc_into_the_forkserver` pins it.
...
- **A refusal is `_unauthorized()`: 401, `WWW-Authenticate: Bearer realm="breakdown"`, and a `detail` that says which thing went wrong.** RFC 6750 reserves the `error` attribute for the case where credentials were presented, so `error="invalid_token"` appears only then, and the body distinguishes *no Authorization header reached the server* from *the token does not match*. The distinction is the diagnostic: a client whose header was stripped in transit (a proxy, a redirect it followed) should be told no token arrived, not sent off to rotate a token the server never saw (#116).
...
| `data` | `GrainedData \| None` | Per-grain frames + `grain_of`/`kind_of`/`last_observed` maps (`last_observed` is captured per metric before the within-grain join; `data_through(m)` converts it to the inclusive last covered date) + `span_of` (each metric's own first and last period; `series(m)` slices to it) + `short_series` (per grain, which metrics fall short of the grain's reach at either edge and by how much) |
...
- **The trace cap is global, and it is a byte budget.** `MAX_CACHED_TRACES = 256` *per tree* would be 256 × N `InferenceData` objects, each holding every posterior draw. One `TraceStore` keyed `(tree_id, metric, fit_end)` is shared, and each tree gets a `TraceView` — a `MutableMapping` speaking the engine's `(metric, fit_end)` key — so `fit_metric` stays a pure function and nothing in `engine/` learns that more than one tree exists. `TraceView.__iter__` snapshots with `list(...)` before filtering, for the same reason `/meta` does (C8): a lazy generator over a dict a worker thread is inserting into raises "dictionary changed size during iteration".
  ...
  ⚠️ **An entry count cannot bound memory, because an entry's size scales with the loaded window.** One **NUTS** fit at the engine's defaults (`NUTS_DRAWS` = 500 draws x `NUTS_CHAINS` = 4 chains) of the demo tree's `order_count` over an 830-day window measures **27.0 MB** of posterior — against 13.4 MB for a 1000-draw ADVI fit and 6.7 MB for an ADVI fit at the same 500 — so 256 of them is ~6.9 GB against `demo/fly.toml`'s `memory = "2gb"`. Re-measured 2026-08-24 when NUTS became the default; the 512 MiB budget now holds ~19 such fits rather than ~76 — and tuning the count down just moves the cliff to a wider window. So the real bound is `MAX_CACHED_TRACE_BYTES` (512 MiB, overridable with `BREAKDOWN_MAX_TRACE_BYTES`; `0` disables the byte bound), with the count kept as a secondary backstop against a pathological number of tiny fits. Eviction is insertion-ordered — oldest first — until **both** bounds fit.
...
**`GET /health`** — always 200: `{status: "ok", provider, metrics, state, data_through, data_through_bounded_by, short_series}` or `{status: "degraded", error}` (the error being `startup_error`, so an auth misconfiguration surfaces here too). `data_through` is the tree-wide as-of date from `_data_through` — the min of the metrics' `data_through`, the one anchor `/health`, the node cards and `_goal_progress` share — so an external monitor can see a serve whose data stopped advancing (GitHub #117: a production serve was down five days with only `/health` polled, and it said ok). It is `null` before a lazy tree's first load and under `provider: none`, and the route never triggers a load to find out. The record carries metric names, ISO dates and counts only, which keeps it inside the C43 rule for this open route. Liveness for orchestrators (the body, not the code, carries degraded-ness) and the UI's first request. Reports on the **default** tree, like every unprefixed route; the per-tree view is `/trees`. One of the three paths that stay open under `BREAKDOWN_REQUIRE_AUTH` — orchestrators cannot present a credential.
...
**`GET /meta`** — `mode` (`"fitted"` | `"cold_start"` — which surface the UI should boot), metrics, data window (null in cold start), provider, per-metric `grains`/`kinds`/`data_through` maps (`data_through` = each metric's honest data edge, which may lag the requested window), `earliest_available` (per-metric earliest provider date from the background discovery task — `{}` until it fills, null per metric when the provider can't say; drives the UI's "history exists before --start-date" nudge), `data_from` (each metric's first covered period — with `data_through`, its own range), `short_series` (per grain, which metrics fall short of the grain's reach and by how much — `{}` when every series agrees; see the loading section above), fitted list (UI bootstrap).
...
**`POST /analyze/{name}`** — `inference_method` (`nuts`|`advi`, default `nuts`), `draws`, `tune`, `chains`, all bounded (50–5000; chains 1–8) and all defaulting to the engine's constants rather than to numbers written here (roadmap C27). Runs `fit_metric` via `asyncio.to_thread` under the lock; stores the trace in `app.state.traces`. **`POST /rca/{name}` and `POST /simulate` take the same `inference_method` with the same default**, so the three fitting routes present one vocabulary rather than three; the alternative — a knob on one and a hardcoded sampler on its neighbours — is the meta-defect the four rules exist for.
...
`/mcp` and `/mcp/` are one endpoint. Starlette's `Mount("/mcp")` compiles to `^/mcp/(?P<path>.*)$`, so a bare `/mcp` misses the mount and falls through to the router's `redirect_slashes` fallback — a 307 to `/mcp/` — and the SDK's sub-app registers an exact `Route("/")`, so nothing short of the trailing slash reaches the transport. Correct for a browser; a trap behind a bearer token, because curl and several HTTP stacks drop `Authorization` on a redirect by design, so the redirected handshake arrived with no token and failed with a 401 that blamed the token rather than the URL (#116). `_McpTrailingSlash`, a pure-ASGI middleware added last (so outermost), rewrites exactly `<root_path>/mcp` to `/mcp/` in the scope before routing; `/mcphony` and `/mcp/anything` are untouched, the router's redirect stays on for every other route, and `bearer_token` sees `/mcp/` either way. It is a scope rewrite rather than `redirect_slashes=False` on the app because that switch is global: `/trees/` would turn from a 307 into a 404.
  ...
  → Parser (Pydantic + NetworkX DAG; nodes carry MetricDefinition incl. grain/kind)
    ...
    → load_tree(tree), on first use: fetch_metric(name, window, grain, kind) per metric
        ...
        → POST /rca/{name}     → run_rca(dag, data, app.state.traces, ...)
                                   ├─ fits missing probabilistic nodes (NUTS by default, node grain)
