...
**Fitted mode** (`data` present): baselines are `window_mean` over `baseline_start/end` (required; whole periods at each node's grain, no whole period → `ValueError`); coefficient draws index each needed node's `beta_raw` posterior, fit on demand with ADVI into the caller's `traces` cache (keyed by `FitKey`, `fit_end = baseline_end + 1 day`, or the full-window `None` when the baseline runs to the data edge). Extrapolation honesty flags compare simulated levels against full-history min/max and ±2σ.

**Cold-start mode** (`data=None` — a tree with no data): the same machinery with zero rows. Baselines come from each node's asserted `baseline` declaration via `_sample_baseline` — `Normal(mu, (high−low)/2z₉₀)` or, with `distribution: lognormal`, a lognormal whose central 90% *is* `[low, high]` — **truncated to the node's declared `plausible` bounds by rejection** (roadmap C7; clipping would pile a fake mode on the boundary, and a belief lying essentially outside its own bound raises rather than looping). Formula nodes derive per-draw from parents in topological order so identities hold under the stated beliefs. The reported point is the MC statistic of those same draws — the **mean**, except where `_unstable_mean` finds it fold-unstable (a ratio whose denominator approaches zero), in which case the **median** is reported and the swap is disclosed in a caveat naming the node; `beta_raw` is sampled directly from each edge's YAML prior in business units (`_sample_prior`, with analytic `_prior_mean` for the Shapley point pass) — the `x_std/y_std` rescaling exists only to reach normalized space for fitting, so with nothing to fit the prior IS the coefficient distribution. Extrapolation flags come from declared `plausible` bounds (no bounds → no flag). The scenario must omit `baseline_start/end`; `traces` is untouched. `validate_cold_start(dag) -> List[str]` returns every blocker — a non-formula node without `baseline`, a probabilistic edge without an explicit prior (the fitted-mode `Normal(0, 1)` fallback is meaningless with no data to set the scale) — and `run_scenario` raises on a non-empty list.

Response contract: `{"mode": "fitted"|"cold_start", "baseline_window" (null in cold-start mode), "n_draws", "seed", "sources", "nodes", "warnings", "caveats"}`. Per-node: `status` (`baseline`|`affected`|`intervened`), `baseline`, `simulated`, `delta: {estimate, ci_95}`, `relative_delta`, `prob_direction`, `fit_quality`, `extrapolation`, `contributions`; cold-start mode adds `baseline_ci_95` (null for point baselines) and swaps `CAVEATS` for `COLD_START_CAVEATS` (plus, when any node fell back to a median point, one appended caveat naming those nodes). Seeded rng (`seed: 0`) with fixed draw order — identical calls are byte-identical.
...
### Trace cache (`FitKey` / `TraceCache`, in `engine/model.py`)

`app.state.traces` is a **`TraceCache`**, not a dict, and is keyed by **`FitKey`** — `(metric, fit_end, inference_method, draws, tune, chains, random_seed)`. Three C8 defects drove both.

- **The old `(metric, fit_end)` key was not an identity.** It said *which series and window*, nothing about *how it was fitted* — so `POST /analyze/x?draws=50&fit_end=D` wrote the entry `run_rca` would reuse for analysis window `D`, and every later `ci_95` on that node became percentiles of 50 unseeded samples. That silently falsified `shaping.py`'s promise that a `report_url` deep link reproduces its numbers. `FitKey` defaults (`advi`, 500 draws, seed 0) *are* the engine's on-demand fit, so `FitKey(node, window)` from `run_rca`/`run_scenario` stays terse while a differently-specified fit gets its own entry.
- **No eviction.** LRU-bounded at `MAX_CACHED_FITS` (256). A 107-metric tree adds ~100 `InferenceData` objects per RCA and ~100 more per additional analysis window; unbounded, a caller could exhaust the process. Evictions are logged and counted (`cache.evictions`); an evicted fit is simply refit on demand.
- **No synchronization.** `run_rca` mutates from a worker thread (`asyncio.to_thread`) while `/meta` and `_pick_fit` iterate — `RuntimeError: dictionary changed size during iteration`, most likely while the UI polls a long RCA. **The existing `asyncio.Lock` could never have fixed this**: it serializes coroutines, and the mutation is off-loop in a thread. `TraceCache` holds a `threading.Lock` and returns a snapshot from `__iter__`/`snapshot()`, so readers are safe *and* do not queue behind a multi-minute fit.
