...
#### PSIS k̂ and escalation (roadmap S2)
...
**Escalation.** `fit_metric(..., escalate_on_khat=True)` discards an `unusable` approximation and re-fits the node with NUTS inside the same model context; `inference_method` then reads `nuts`, `khat_status` reads `escalated`, `khat` keeps the *rejected* fit's value, and `escalated_from` names the method that was thrown away. Default **off**, because `inference_method` is a promise about which sampler runs and the primitive must keep it (the S1 benchmark and the triage-speed path both depend on that). The callers that chose ADVI on the *user's* behalf — `run_rca` and `run_scenario` — turn it on. `POST /analyze/{name}?inference_method=advi` deliberately does not: there the caller asked for ADVI, and the remedy is `?inference_method=nuts`.

**The cap is rule 4's shape.** `MAX_ESCALATIONS = 4` lives in `model.py` and is imported by both orchestrators so the two cannot drift; each holds its own `escalations_left` counter **on the stack**, so `run_rca` stays a pure function of its arguments and two concurrent analyses cannot spend each other's budget. Above the cap a flagged node keeps its approximation and gains `escalation_budget_warning(...)` — which names the cap *and* the per-node NUTS remedy, so it is distinguishable from "the caller never asked for escalation". Four covers every tree in this repo (White Cube fits four probabilistic nodes) while bounding the worst case at roughly four minutes of sampling behind one lock rather than ten.
...
1. **Fit what's missing.** Probabilistic (non-formula, non-root) nodes in scope without a trace are fitted with ADVI — skipped when their windows hold no whole period at their grain — with `escalate_on_khat` on and `MAX_ESCALATIONS` as the budget (above). An escalated node's NUTS fit is cached under the same `(node, analysis_start)` key, so a repeat analysis of the same window pays nothing and the *next* analysis has its full budget for whatever is still an approximation.
...
Response contract: `{"mode": "fitted"|"cold_start", "baseline_window" (null in cold-start mode), "n_draws", "seed", "sources", "nodes", "warnings", "caveats"}`. Per-node: `status` (`baseline`|`affected`|`intervened`), `baseline`, `simulated`, `delta: {estimate, ci_95}`, `relative_delta`, `prob_direction`, `fit_quality`, `khat_status`, `khat_warnings`, `extrapolation`, `contributions`; cold-start mode adds `baseline_ci_95` (null for point baselines) and swaps `CAVEATS` for `COLD_START_CAVEATS`. Seeded rng (`seed: 0`) with fixed draw order — identical calls are byte-identical.
  ...
  → Parser (Pydantic + NetworkX DAG; nodes carry MetricDefinition incl. grain/kind)
    ...
    → load_tree(tree), on first use: fetch_metric(name, window, grain, kind) per metric
        ...
        → POST /rca/{name}     → run_rca(dag, data, app.state.traces, ...)
                                   ├─ fits missing probabilistic nodes (ADVI + k̂ escalation, node grain)
