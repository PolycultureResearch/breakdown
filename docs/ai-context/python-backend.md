...
**No escalation, and that is the finding rather than an omission.** The first cut of S2 re-fitted an `unusable` node with NUTS behind the caller's back, capped at four per analysis. Then the measurement came in: mean-field fails the check on *essentially every real node* (10.18 / 1.07 / 0.85 / 1.36 on White Cube's four, 1.04 on jaffle's one, 1.26 / 0.88 / 1.11 / 10.43 on a second White Cube window), which made "escalation" the common path rather than a rescue — and the cost argument that justified ADVI collapsed alongside it (the 106-metric B2B tree holds exactly five probabilistic fits, and fitting all five measured 31.3s ADVI against 28.0s NUTS — the exact sampler was the faster of the two). So the default moved to NUTS everywhere and the escalation machinery came out: `MAX_ESCALATIONS`, `escalation_budget_warning`, `escalate_on_khat`, the `escalated` status and the "re-fitted with NUTS" UI badge are all gone.
