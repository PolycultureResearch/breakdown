...
`build_grained` then requires each grain frame to be a **gap-free run of periods** (`_check_contiguous`, grain-aware via `_FREQ`), raising with up to 10 named missing dates plus a count. This is not tidiness: everything downstream indexes by position — the model's `t = arange(len(y))` dates the rows, lags shift by *rows*, and the bootstrap resamples contiguous runs — so a hole compresses the calendar and silently shifts every date rather than failing.

**What the inner join drops at the edges is named, with the metric responsible** (`_report_clipping`, GitHub #112). A series shorter than its siblings at either end passes the contiguity check — the survivors are still a gap-free run — while bounding the shared frame for every metric at the grain. That is what a stale feed looks like (`_align_to_spine` trims a trailing gap per series, correctly, and the join then applies the shortest trim to everyone), and in 0.2.0 it clipped a production tree's day grain to one period with the tree loaded, `/health` ok, and every RCA failing later with "reference window not fully covered". The reporter compares each metric's own dates (captured before the join, next to `last_observed`) against the joined range, and for each clipped edge records the metrics tied at that edge (`by`), the shared edge (`clipped_to`), the furthest edge any sibling reached (`others_reached`) and the count of whole periods lost (`periods_dropped`), onto `GrainedData.grain_clipping` keyed by grain — JSON-safe, since it is a disclosure and not an input to arithmetic. It logs **one WARNING per grain**, both edges in one line, worded after the issue's own request ("day grain clipped to 2026-08-08 by `paid_spend` (other series ran to 2026-08-26) …") and ending with the remedy. The join itself is unchanged and nothing is filled: this is rule 1, the boundary naming what it dropped rather than approximating. `/meta` carries the record as `grain_clipping` (always present, `{}` when clean) and MCP `get_tree` carries it only when non-empty. Per-metric windows — the fix that would let the late node narrow only its own window — are the issue's second suggestion and remain open.
...
| `data` | `GrainedData \| None` | Per-grain frames + `grain_of`/`kind_of`/`last_observed` maps (`last_observed` is captured per metric before the within-grain join; `data_through(m)` converts it to the inclusive last covered date) + `grain_clipping` (per grain, what that join cut at either edge and which metric bounded it) |
