...
### `fit_metric(dag, data, target, draws=1000, tune=1000, inference_method="nuts", fit_end=None, chains=4, random_seed=None, vi_iterations=20_000) -> FitResult`
...
### `run_rca(dag, data, traces, target, *, analysis_start, analysis_end, reference_start=None, reference_end=None, inference_method="nuts", draws=500)`
...
### `run_scenario(dag, data, traces, scenario, inference_method="nuts", draws=500, n_draws=2000)`
...
**`POST /analyze/{name}`** — `inference_method` (`nuts`|`advi`, default `nuts`), `draws`, `tune` (50–5000). Runs `fit_metric` via `asyncio.to_thread` under the lock; stores the trace in `app.state.traces`. **`POST /rca/{name}` and `POST /simulate` take the same `inference_method` with the same default**, so the three fitting routes present one vocabulary rather than three; the alternative — a knob on one and a hardcoded sampler on its neighbours — is the meta-defect the four rules exist for.
