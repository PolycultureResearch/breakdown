...
- `_ppc_diagnostic(y, y_rep, mu, target, grain)` — roadmap S3, described under its own heading below. The draws come from `_posterior_predictive_draws(...)`, which must run **inside** the `pm.Model()` context; the verdict is attached afterwards as `ppc_status` / `ppc` / `ppc_warnings`.
