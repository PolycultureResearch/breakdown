...
| `plausible` | dict | **Cold-start mode only.** Declared honesty band `{min, max}` (either bound may be omitted, at least one required). Belief draws are **truncated** to it at sampling time (rejection resampling, so no mass piles up on the bound — `min: 0` means customer counts cannot be drawn negative), and it stands in for historical min/max in the what-if extrapolation flags. See [Cold-start mode](#cold-start-mode-what-if-with-no-data). |
