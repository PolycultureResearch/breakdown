...
- **BigQuery execution for the `dbt` provider** (`pip install 'metric-breakdown[bigquery]'`). The generated SQL was already BigQuery-shaped — the dialect and its ISOWEEK week bucketing shipped with roadmap 2.10 — but there was no connector, so a BigQuery shop was pushed onto `local` and its `mf` subprocess. The profile's `method` is honoured: `oauth` (Application Default Credentials), `service-account` (`keyfile`), and `service-account-json` (`keyfile_json`); an unsupported method is refused by name rather than falling back to ADC as somebody else. Supported adapters are now BigQuery, Databricks, DuckDB, Postgres and Snowflake.
