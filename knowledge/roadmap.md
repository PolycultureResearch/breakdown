...
(C1–C5 shipped — every provider now shares one date-alignment contract, RCA
publishes the exact Shapley value, the window bootstrap's finite-sample
attenuation is corrected, and `ranked_causes` no longer inverts on offsetting
noise), remaining statistical rigor (T11), UI trust finish
...
the defect, S12 is about the prominence. **C6 is next**, and is a one-line
validator.
