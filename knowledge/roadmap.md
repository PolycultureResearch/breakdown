...
| C32 | **The slice and flow caches are bounded by entry count, and the thing that grows is cardinality** — a high-cardinality dimension (830 days × 5,000 slice values) is 154 MB per cached raw frame, ×64 entries against the demo's 2 GB box; the generated `GROUP BY` has no LIMIT and `top_k` is applied only after the whole frame is resident. Byte bound shipped 2026-08-31; **the SQL half shipped 2026-09-14**: the `dbt` provider folds everything outside `top_k`/`values` into `__other__` in the generated query, ranked over the two windows with ties returned raw, and both paths are held to the same frame on the same data — the row's own case measured 132.8 MB / 4.15M rows whole against 0.3 MB / 9,960 rows rolled. *(full history: [C32](roadmap_log.md#c32))* | ✅ | Rule 2's own words: bound by the thing that actually grows — the invariant test checked the cache's *type* and never its bound |
...
| 1.7 | **Per-node aggregation grain** — let a node declare its natural grain (daily flows, weekly/monthly cohort rates); resample each node to its grain before fit/attribution instead of forcing daily. **Per-metric windows followed (2026-09-15, GitHub #112):** the within-grain join is outer, each metric keeps its own range, and a fit's window is the intersection of the node's range and its parents' — a short series bounds only the analyses that read it. *(full history: [1.7](roadmap_log.md#1-7); per-metric windows: [per-metric-windows](roadmap_log.md#per-metric-windows))* | ✅ | Ratio/cohort nodes are degenerate at daily grain (ARPU on a 1-member day; conversion on a low-volume day), producing noise the bootstrap then papers over. |
...
| S23 | **Reference-window sensitivity** — every number the engine publishes is a contrast of two window means, and the only uncertainty ever quantified is *within*-window sampling; nothing resampled the choice of window, which is usually the engine's own. **Shipped 2026-09-14:** every `run_rca` re-attributes under two neighbouring reference blocks over the same fits and publishes `reference_sensitivity` — whether the top cause and the gap's direction survive the move, the alternatives tried, and a `gap_range` kept deliberately outside `ci_95` — on the API, MCP and both UI surfaces. *(full history: [S23](roadmap_log.md#s23))* | ✅ | The module scrupulous enough to censor a direction probability at 1 − 1/500 said nothing about the single input with the largest influence on its headline answer; now a reader can see whether "X is the top cause" survives moving the reference window by one week |
| S24 | **Known, dated interventions — a declared step/pulse term in the fit** — a node declares `interventions:` (name, date, `kind: step\|pulse`, optional business-unit prior), each a known 0/1 regressor on its own `beta_intervention` axis (never appended to `beta`, whose order is the parent list), learned by default from instances inside the fit window and dropped by name when there are none; an explicit per-intervention `learn_from: window` opt-in lets the fit see the event's own periods (Box–Tiao intervention analysis) with the claim stated on the payload, and a per-node `fit_start` beside it. Design: [step_change_design.md](step_change_design.md); the marketing-lift document's "event-term fit-window exception" is this row's `window` mode, not a second item | ○ | Raised by issue #114 (a ticketed-event tree: price flips, on-sale and announce days are steps, and a local level cannot take a step, so the S3 check says `severe` and the practitioner has nowhere to go inside the tool) and independently by a marketing team the same day (a campaign flag that lives only in the analysis window is dropped, correctly, and its effect lands in `unexplained`). Ahead of S8 on value: momentum is not what either PPC is flagging |
