...
| 2.12 | **`bsl` binding** (additive, off the critical path) — Boring Semantic Layer over **plain Ibis**, skipping xorq and BSL profiles (the tested path, added upstream for "Databricks backend unsupported in xorq"), all calls behind one adapter module and pinned exactly | ○ | Serves hand-authored BSL YAML, and is the only way to get `breakdown serve` running over a local parquet file with no warehouse and no credentials — currently impossible, and the reason the White Cube demo (3.7) ships committed snapshots instead of querying anything. Small: one module plus an adapter. BSL has **no dbt interop of any kind**, so it is not a route to 2.10 |
| 2.13 | **Retire the `local` provider** — mark superseded once 2.10 has real mileage, README migration note, keep working and tested until a removal release is named separately. `cloud` is **not** superseded | ○ | 2.10 does everything `local` does — same dbt project on disk, same no-dbt-Cloud story — but in-process, with multi-dimension grouping, SQL provenance and dimension discovery, and without the `mf` binary that breaks the extra on 3.14. Two providers solving one problem, one of them by subprocess-and-CSV, is a maintenance and docs cost with no user on the other side |
...
| 3.8 | **Non-additive decomposition at entity grain** — `count(distinct …)` and semi-additive stocks are not additive across slices, but are identically `sum(1)` over an entity × period presence table. With a declared `entity_key` (+ optional entity-grain relation) this yields growth accounting — Δusers = new + resurrected − churned, an *exactly* additive identity where each user lands in exactly one bucket — and stock-and-flow for balances. Three tiers: entity-grain relation → full decomposition; entity key only → per-slice Δ with overlap named as an explicit residual (roughly what ships today); neither → trend only, contribution percentages refused. Needs its own design doc; contract hooks land with [2.9](#horizon-2--make-it-repeatable-a-stranger-can-onboard) | ○ | Today a user switching platforms shows −1 on A and +1 on B with the total unchanged, and naive slice attribution reports two large offsetting "causes" for a change that never happened. Entity flows label it as *migration* — and migration is frequently the real finding, not noise to be explained away. This is the same posture as [3.2](#horizon-3--make-it-findable-and-sticky-it-comes-to-you)'s refusal to rescale non-summing slices, carried from *reporting* the residual to *decomposing* it. General rule: a non-additive metric requires a declared entity, and decomposition happens at the grain where the metric becomes a sum |
