...
(C1/C2 shipped — every provider now shares one date-alignment contract),
remaining statistical rigor (T11), UI trust finish (U5–U6), the connectivity kit,
and the market-driven items (scheduled monitoring, the rest of dimensional
slicing).
...
modes now have tests that fail against the previous code; **C3 is next.**
