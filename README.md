...
> Revenue is up, but by much less than order volume alone would have delivered: order count rose and average order value fell, and the AOV drag is by far the more certain of the two effects.
>
...
> - **Order count is the mover.** 142.7 → 148.0 orders/day (+3.8%), worth +$985/day of that gap. That is 165% of the gap on its own; shares are unclamped here, and this one exceeds 100% precisely because something else is pulling the other way.
> - **Average order value is that something else.** $184.68 → $182.15 (−1.4%), worth −$367/day, or −62% of the gap. This is the confident half of the story: the 95% credible interval is −$506 to −$207 and the direction is certain (`prob_same_direction` 1.00). The order-count lift is the unconfident half — its interval runs −$52 to +$2,088, so the sign is likely (0.97) but the magnitude is not pinned down, and I would not quote the +$985 without that caveat.
> - Sessions were up 2.2%, and the fitted model puts a bit over half the order-count gain on them (+3.0 of the +5.4 orders/day). But that leg sits at 0.85 probability on the direction with an interval that crosses zero — suggestive, not established.
> - The unexplained remainder is −$22/day against a $596 gap, under 4%, so the order-count-versus-AOV story accounts for essentially the whole move.
>
> This tree can't say *why* AOV fell — nothing upstream of it is modeled — so the next step is outside the tree: discounting and promos, product mix, or a shift toward cheaper SKUs in that window. One caveat on the question: the loaded data only covers Jan 1 – Apr 9, 2024, so "the last two weeks" means the most recent two weeks of available data, not the end of the year.
>
...
Every claim there traces back to a field in the tool response. Order count's 165% share is the unclamped `share_of_gap` design at work — the number is only interpretable *because* it is allowed past 100%. "Confident" and "suggestive" are `ci_95` and `prob_same_direction` read as confidence language, applied per contribution rather than flattened into one voice for the whole answer. The small `unexplained` is cited as evidence the story is complete, which is the only thing that entitles the answer to say "essentially the whole move". And "this tree can't say why AOV fell" is the DAG-is-a-hypothesis caveat from `how_to_read`, applied where it bites instead of appended as a disclaimer. The closing link replays the exact analysis in the UI.

One thing the narration deliberately does *not* do: quote `ranked_causes`. It ranks `order_count` first with a score of exactly **1.0**, which looks like certainty and is not — the score saturates at 1 whenever a parent's share exceeds the gap, as this one's 165% does. Use the ordering, not the number; see [`ranked_causes` is a heuristic](https://github.com/PolycultureResearch/breakdown/blob/main/docs/model.md) in `docs/model.md` for the open defect behind it.
